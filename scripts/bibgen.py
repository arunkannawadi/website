...
if os.path.exists('_data/custom_bib.yml'):
...
encoded_query='q=author:Kannawadi,+A&fl=bibcode,citation_count&rows=1000&sort=date+desc'
...
# Set select = {true} for selected publications
selected = customization.pop("selected", [])
...
for entry in bibtex_database.entries:
  ...
  if entry['ID'] in customization.get("major", []):
      entry["custom_keywords"] = "major"

  if entry['ID'] in customization.get("white", []):
      entry["custom_keywords"] = "white"

  if entry['ID'] in customization.get("first", []):
      entry["custom_keywords"] = "first"
...
with open("_includes/metrics/ads_h.md", "w") as f:
  f.write(str(h_index))

with open("_includes/metrics/ads_citedby.md", "w") as f:
